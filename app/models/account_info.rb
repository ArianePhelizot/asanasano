...
COUNTRY_LIST = %w(AD AE AF AG AI AL AM AO AQ AR AS AT AU AW AX AZ BA
  ...
  validates :first_name, :last_name, :birthday, :country_of_residence, :nationality, presence: true, if: :natural?
  ...
  validates :country_of_residence, :nationality, inclusion: { in: COUNTRY_LIST}, if: :natural?
  validates :legal_representative_country_of_residence, :legal_representative_nationality,  inclusion: { in: COUNTRY_LIST}, if: :legal?
  ...
  def default_values
    self.first_name ||= User.find(self.user_id).first_name
    self.last_name ||= User.find(self.user_id).last_name
    ...
    if User.find(self.user_id).coach_id
